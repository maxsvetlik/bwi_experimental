...
#include <jaco_msgs/JointAcceleration.h>
...
class JacoArm
{
 ...
 private:
    ...
    ros::Publisher tool_position_publisher_; //publishes tool position as (x,y,z) and a quaterneon for rotation
    ros::Publisher cartesian_position_publisher_; //publishes position as (x,y,z) and rotation as (theta_x, theta_y, theta_z)
    ros::Publisher finger_position_publisher_;
    ros::Publisher joint_state_publisher_; //publishes joint values that match the RViz URDF model
    ros::Publisher joint_effort_publisher_;
    ros::Publisher joint_accel_publisher_;
